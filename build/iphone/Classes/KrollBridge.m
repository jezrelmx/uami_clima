...
NSString *Clima_CDMX$ModuleRequireFormat = @"(function(exports){"
...
@implementation Clima_CDMXObject
...
{
  // create Clima_CDMX global object
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];

  // Load the "Clima_CDMX" object into the global scope
  NSString *basePath = (url == nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
  _clima_cdmx = [[Clima_CDMXObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

  TiContextRef jsContext = [kroll context];
  TiValueRef tiRef = [KrollObject toValue:kroll value:_clima_cdmx];

  NSString *_clima_cdmxNS = [NSString stringWithFormat:@"T%sanium", "it"];
  TiStringRef prop = TiStringCreateWithCFString((CFStringRef)_clima_cdmxNS);
  ...
  if (preload != nil) {
    for (NSString *name in preload) {
      KrollObject *ti = (KrollObject *)[_clima_cdmx valueForKey:name];
...
- (KrollWrapper *)loadCommonJSModule:(NSString *)code withSourceURL:(NSURL *)sourceURL
{
  ...
  NSString *js = [[NSString alloc] initWithFormat:Clima_CDMX$ModuleRequireFormat, dirname, filename, code];
...
- (KrollWrapper *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
  ...
  if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
    @throw [NSException exceptionWithName:@"org.clima_cdmx.kroll"
