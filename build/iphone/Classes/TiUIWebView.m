...
static NSString *const kClima_CDMXJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
- (NSString *)_clima_cdmxInjection
{
  ...
  [html appendString:kClima_CDMXJavascript];
...
+ (NSString *)content:(NSString *)content withInjection:(NSString *)injection
{
  ...
  // attempt to make well-formed HTML and inject in our Clima_CDMX bridge code
             ...
             baseURL:(NSURL *)baseURL
{
  ...
  content = [[self class] content:content withInjection:[self _clima_cdmxInjection]];
            ...
            mimeType:(NSString *)mimeType
{
  ...
  [request setValue:[NSString stringWithFormat:@"%lu", (localId++)] forHTTPHeaderField:@"X-Clima_CDMX-Local-Id"];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
  ...
  if ([scheme hasPrefix:@"http"] || [scheme isEqualToString:@"ftp"]
    ...
    if ([scheme isEqualToString:@"file"] || [scheme isEqualToString:@"app"]) {
      [LocalProtocolHandler setContentInjection:[self _clima_cdmxInjection]];
    }
    return YES;
  }
