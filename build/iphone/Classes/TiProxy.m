...
//Rare exceptions to indicate a bug in the _clima_cdmx code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString *)qualifiedName withProperties:(NSDictionary *)properties inContext:(id<TiEvaluator>)context
{
  ...
  if (proxyClass == nil) {
    NSString *_clima_cdmx = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium."];
    if ([qualifiedName hasPrefix:_clima_cdmx]) {
    ...
    if (proxyClass == nil) {
      ...
      @throw [NSException exceptionWithName:@"org.clima_cdmx.module"
