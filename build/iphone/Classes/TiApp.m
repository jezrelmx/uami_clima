...
- (NSString *)systemUserAgent
{
  ...
  NSString *kClima_CDMXUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s", "Appc", "eler", "ator", "Tita", "nium"];
  return [NSString stringWithFormat:@"%@/%s (%@)", kClima_CDMXUserAgentPrefix, TI_VERSION_STR, currentDeviceInfo];
}
...
+ (NSDictionary *)tiAppProperties
{
  ...
  if (props == nil) {
    ...
    if (jsonData) {
      ...
      // If we have no data...
      // This should never happen on a Clima_CDMX app using the node.js CLI
